      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center text-slate-600 gap-x-6 gap-y-2">
            <span className="flex items-center gap-1"><Mail className="h-4 w-4" /> {auctioneer.contacts?.email || "N/A"}</span>
            <span className="flex items-center gap-1"><Calendar className="h-4 w-4" /> Joined {new Date(auctioneer.created_at).toLocaleDateString()}</span>
            {auctioneer.contacts?.phone && (
              <span className="flex items-center gap-1"><Phone className="h-4 w-4" /> {auctioneer.contacts.phone}</span>
            )}
          </div>
        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      {auctioneer.contacts.website ? (
                        <a href={auctioneer.contacts.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1 font-medium">
              ...
              {auctioneer.bank ? (
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <Badge variant="outline" className="mt-1 capitalize bg-slate-50">
            ...
            <CardContent className="pt-6">
              <div className="grid sm:grid-cols-2 gap-4">
                {(auctioneer.documents?.length ?? 0) > 0 ? auctioneer.documents.map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center gap-3">
              ...
              {showRejectForm ? (
                ...
                <div className="flex flex-col gap-3">
                  <Button 
                    ...
                    disabled={auctioneer.status === "approved" || approveAuctioneer.isPending}
                  >
                    <CheckCircle className="h-5 w-5 mr-2" /> Approve Application
                  </Button>
                  <Button 
                    variant="outline"
                    className="text-blue-600 border-blue-200 hover:bg-blue-50 w-full py-6"
